...
for output_page in $PAGES; do
  ...
  sed -i.tmp -e "s\`%NAVBAR_ITEMS%\`$navbar\`" -e 's# />#>#' -e "s/%PANDOC_VERSION%/$PANDOC_VERSION/"\
             ...
             -e 's@%CAROUSELBUTTON(\([^,(]*\), \([^,]*\), \([^,]*\)\(,\(.*\)\)\?)%@      <input class="btn-check" id="\2\3" name="\2" type="radio" role="tab" aria-controls="\2Tab\3"\5>\n      <label class="btn btn-outline-primary nav-link" for="\2\3">\1</label>@'\
             -e 's@%CAROUSELIMAGE(\([^,(]*\), \([^,]*\), \([^,]*\), \([^,]*\))%@    <div class="tab-pane fade show" id="\2Tab\3" role="tabpanel">\n      <picture>\n        <source srcset="assets/\4.avif" type="image/avif">\n        <source srcset="assets/\4.webp" type="image/webp">\n        <img src="assets/\4.png" class="d-block img-fluid m-auto w-auto" alt="\1">\n      </picture>\n    </div>@'\
             -e '/<!-- TODO: .*-->/d'\
